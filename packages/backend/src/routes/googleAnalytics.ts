...
router.get('/test/oauth-url', (req, res) => {
  try {
    const state = req.query.state as string || 'test123';
...
router.get('/test/metrics', async (req, res) => {
  try {
    ...
    const metrics = await googleAnalyticsService.getBasicMetrics(
...
router.get('/metrics', authenticateToken, async (req, res) => {
  try {
    ...
    // Validate property ID format
    if (!googleAnalyticsService.validatePropertyId(propertyId as string)) {
      return res.status(400).json({
        success: false,
        error: 'Invalid GA4 property ID format'
      });
 ...
 * @access Private
 */
router.get('/health', authenticateToken, (_req, res) => {
  try {
    // Check if required environment variables are set
    const requiredEnvVars = [
      'GOOGLE_CLIENT_ID',
